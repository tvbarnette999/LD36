...
public class TechTree extends Overlay {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5070463450301859743L;
	...
	static Tech shoeLaces = new Tech("Shoe Laces", "Keeps runners shoes from coming off, doubling speed", 2,
			Transport.RUNNER);
	static Tech shoes = new Tech("Shoes", "Increases Runner Speed by 30%.", 1.3, Transport.RUNNER);
	static Tech agriculture = new Tech("Agriculture", "Better Food Increases Runner payload by 40%", 1.4,
			Transport.RUNNER);
	static Tech paper = new Tech("Paper", "Messages on paper weight less than stone, tripling Runner payload", 3.0,
			Transport.RUNNER);
	...
	static Tech leather = new Tech("Leather", "Better clothing, increases runner payload by 10%", 1.1,
			Transport.RUNNER);
	static Tech snacks = new Tech("Snacks", "Runners can eat while running, increasing speed by 20%", 1.2,
			Transport.RUNNER);
	...
	static Tech horses = new Tech("Horses", "Horses are much faster than Runners!", Transport.HORSE_TYPE,
			Transport.currentUnits, Transport.HORSE);
	...
	static Tech horseFood = new Tech("Horse Food", "Who knew you had to feed them???", 70, Transport.HORSE,
			Transport.CARRIAGE, Transport.CARAVAN);
	static Tech horseShoes = new Tech("Horseshoes", "Increases horse speed by 30%", 1.3, Transport.HORSE,
			Transport.CARRIAGE, Transport.CARAVAN);
	static Tech wheel = new Tech("The Wheel", "They see me rollin'. \n \nThey hatin'.", 1.5, City.class); // so
																											// that
																											// it
																											// does
																											// something
	static Tech weights = new Tech("Weight Training", "Do Runners even lift?. Increase payload by 200%", 3,
			Transport.RUNNER);
	static Tech breeding = new Tech("Enhanced Breeding", "Horses are faster and stronger", 15, Transport.HORSE,
			Transport.CARAVAN);
	static Tech dirtRoad = new Tech("Dirt Roads", "Easier for all units to traverse", 1, Road.DIRT);

	static Tech olympics = new Tech("Olympics", "Competitive Running! \n \nNational Pride!", 2.1, Transport.RUNNER,
			City.class);
	static Tech relays = new Tech("Relays", "Runner better at delivering things.", 3.1, Transport.RUNNER);
	static Tech carrot = new Tech("Carrot on a Stick", "Motivated horses to move faster", 17.27589, Transport.HORSE,
			Transport.CARRIAGE, Transport.CARAVAN);
	static Tech spurs = new Tech("Spurs", "Better control of horses", 1.5, Transport.HORSE);
	static Tech ironHorseShoes = new Tech("Iron Horse Shoes", "Increase horse speed by 350%", 4.5, Transport.HORSE,
			Transport.CARRIAGE, Transport.CARAVAN);
	static Tech smithing = new Tech("Smithing", "Make cool stuff", 2.1, City.class);
	static Tech smootherRoads = new Tech("Smoother Roads", "Runners and horses dont trip.", 4, Transport.RUNNER,
			Transport.HORSE, Transport.CARAVAN, Transport.CARRIAGE);
	static Tech ditches = new Tech("Ditches", "Drier roads, Runners and horses don't get stuck in mud anymore.", 3,
			Transport.RUNNER, Transport.HORSE, Transport.CARAVAN, Transport.CARRIAGE);
	...
	static Tech medicine = new Tech("Medicine", "Heal sick runners and horses, instead of shoot them!", 6,
			Transport.RUNNER, Transport.HORSE, Transport.CARAVAN, Transport.CARRIAGE);
	// static Tech literacy = new Tech("Literacy", "");
	static Tech steel = new Tech("Steel", "Stronger than iron, leads to way cooler stuff. (Like railroads)", 1.1,
			City.class);
	// static Tech steelFarm = new Tech("Steel Farm Equipment", "");
	static Tech math = new Tech("Math", "42", 4.2, City.class);
	static Tech carriage = new Tech("Horse Drawn Carriage", "Way more space to store stuff", Transport.CARRIAGE_TYPE,
			Transport.currentUnits, Transport.CARRIAGE);
	static Tech metalRims = new Tech("Metal Rims", "Adds coolness, so increases carriage speed by 100%", 2,
			Transport.CARRIAGE);
	static Tech lighterCarriage = new Tech("Lighter Carriage",
			"No more mother-in-laws. \nIncrease crriage speed by 2000%", 20, Transport.CARRIAGE);
	static Tech twoHorse = new Tech("Two Horse Carriage", "Double the horses triples the speed.", 3,
			Transport.CARRIAGE);
	static Tech caravan = new Tech("Caravan", "Groups of carriages carry way more", Transport.CARRIAGE_TYPE,
			Transport.currentUnits, Transport.CARAVAN);
	...
	static Tech catapult = new Tech("Catapult", "Wheeeeeeeeeeee!", Transport.CATAPULT_TYPE, Transport.currentUnits,
			Transport.CATAPAULT); // TODO
									// CATAPULTS!*******************************************************
	...
	static Tech onager = new Tech("Onager", "Bigger payload and higher distance", 2, Transport.CATAPAULT_RANGE,
			Transport.CATAPAULT);
	...
	static Tech railroad = new Tech("Rail Roads", "CHOO CHOO!!!", Transport.TRAIN_TYPE, Transport.currentUnits,
			Transport.TRAIN, Road.RAIL);
	...
	static Tech horseSteroids = new Tech("Horse Steroids", "Enhances horse performance by 300%", 3, Transport.HORSE,
			Transport.CARAVAN, Transport.CARRIAGE);
	...
	static Tech cowCatchers = new Tech("Cow Catchers", "Gets stuff out of the way, increasing speed by 10%", 1.1,
			Transport.TRAIN);
	...
	static Tech cars = new Tech("Cars", "Can transport way more than caravans. Need paved roads", Transport.CAR_TYPE,
			Transport.currentUnits, Transport.CAR);
	...
	static Tech jetFuel = new Tech("Jet Fuel", "High quality fuel. Might can use it in the future.", 75,
			Transport.PLANE);
	static Tech beams = new Tech("Steel Beams", "Do they melt?", 3.987, City.class);
	static Tech aerodynamics = new Tech("Aerodynamics", "Speeds up cars and trains by 10%.", 1.1, Transport.CAR,
			Transport.TRAIN, Transport.SEMI, Transport.TRUCK);
	static Tech aviation = new Tech("Aviation", "Flying looks cool", 1.2, City.class);
	static Tech airplanes = new Tech("Airplanes", "Flying is fun!", Transport.PLANE_TYPE, Transport.currentUnits,
			Transport.PLANE);
	...
	static Tech trucks = new Tech("Trucks", "Haul twice as much as cars", Transport.CAR_TYPE, Transport.currentUnits,
			Transport.TRUCK);
	...
	static Tech battery = new Tech("Batteries", "Road vehicles can run longer", 1.2, Transport.CAR, Transport.TRAIN,
			Transport.SEMI, Transport.TRUCK);
	static Tech semi = new Tech("Semi-Trailer Truck", "Carries 10x as much as trucks", Transport.CAR_TYPE,
			Transport.currentUnits, Transport.SEMI);
	static Tech electricCar = new Tech("Electric Car", "More efficient that gas", 1.2, Transport.CAR, Transport.TRUCK,
			Transport.SEMI);

	static Tech EPA = new Tech("EPA", "Demands fuel efficiency", 1.2, Transport.CAR, Transport.TRUCK, Transport.SEMI);
	static Tech efficiency = new Tech("Fuel Efficiency", "Regulations require even more efficiency. ", 1.2,
			Transport.CAR, Transport.TRUCK, Transport.SEMI);

	static Tech AI = new Tech("AI", "Its not self aware. Yet.", 1.5, City.class);

	static Tech selfCar = new Tech("Self Driving Car", "Twice as efficient", 2, Transport.CAR, Transport.TRUCK,
			Transport.SEMI);
	...
	static Tech singularity = new Tech("Singularity",
			"Machines take over. Mail is no longer needed. Have infinite Money", java.lang.Double.POSITIVE_INFINITY,
			LD36.theLD);
	// TODO research inflation. you make more monezzz
	// reseach capitalism
	// static Tech onagers = new Tech("Onagers", "Increases catapault range by
	// 20%", Transport.CATAPAULT_RANGE);
	...
	public void draw(Graphics2D g) {
		...
		// }
		//
		// for(int x = 0; x < MAX_COLUMN * (Tech.WIDTH+X_GAP);
		// x+=Tech.WIDTH+X_GAP){
	...
	// super.mouseMoved(new MouseEvent(e.getComponent(), e.getID(), e.getWhen(),
	// e.getModifiers(), e.getX()+(int)scroll, e.getY(), e.getXOnScreen(),
	// e.getYOnScreen(), e.getClickCount(), e.isPopupTrigger(), e.getButton()));
	// }
	// public void mousePressed(MouseEvent e){
	// super.mousePressed(new MouseEvent(e.getComponent(), e.getID(),
	// e.getWhen(), e.getModifiers(), e.getX()+(int)scroll, e.getY(),
	// e.getXOnScreen(), e.getYOnScreen(), e.getClickCount(),
	// e.isPopupTrigger(), e.getButton()));
	// }
	// public void mouseReleased(MouseEvent e){
	// super.mouseReleased(new MouseEvent(e.getComponent(), e.getID(),
	// e.getWhen(), e.getModifiers(), e.getX()+(int)scroll, e.getY(),
	// e.getXOnScreen(), e.getYOnScreen(), e.getClickCount(),
	// e.isPopupTrigger(), e.getButton()));
	// }
	// public void mouseClicked(MouseEvent e){
	// super.mouseClicked(new MouseEvent(e.getComponent(), e.getID(),
	// e.getWhen(), e.getModifiers(), e.getX()+(int)scroll, e.getY(),
	// e.getXOnScreen(), e.getYOnScreen(), e.getClickCount(),
	// e.isPopupTrigger(), e.getButton()));
	// }
