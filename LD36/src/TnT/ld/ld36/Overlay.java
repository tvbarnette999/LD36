...
/**
 * Each type of overlay element is an Overlay object, and each one overrides
 * whichever listener methods it needs calling super. When one uses the event,
 * it needs to consume it so elements further up the hierarchy know its not
 * actually theirs.
 * 
 * @author Thomas
 *
...
public class Overlay extends Rectangle2D.Double implements MouseListener, MouseMotionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1799837536231919324L;
	...
	public void mouseExited(MouseEvent e) {
		...
		for (Overlay o : elements) {
			if (o.mouseIn) {
				o.mouseIn = false;
				o.mouseExited(new MouseEvent(LD36.theLD.panel, MouseEvent.MOUSE_EXITED, e.getWhen(), 0, e.getX(),
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (target != null) {
			if (!target.mouseIn) {
				target.mouseIn = true;
				target.mouseEntered(new MouseEvent(LD36.theLD.panel, MouseEvent.MOUSE_ENTERED, e.getWhen(), 0, e.getX(),
						e.getY(), e.getXOnScreen(), e.getYOnScreen(), 0, false, 0));
			}
			target.mouseMoved(e);
		}
